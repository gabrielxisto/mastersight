...
export default async function (fastify: FastifyInstance) {
  fastify.get("/roles", async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const departments = await prisma.companiesDepartments.findMany({
        where: { companyId: Number(companyId) },
      });
      const roles = await prisma.companiesRoles.findMany({
        where: { companyId: Number(companyId) },
      });
      ...
      const data = roles.map((role: Role) => {
        const roleDepartment = departments.find(
          (department) => department.id === role.departmentId,
        );
  ...
  fastify.post(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { companyId, name, departmentId, salary } = req.body as {
          companyId: number;
          name: string;
          departmentId: number;
          salary: string;
        ...
        if (!salary || formattedSalary < 0)
          return reply.status(400).send({ error: "invalid-salary" });
  ...
  fastify.post(
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { id, companyId, name, departmentId, salary } = req.body as {
          id: number;
          companyId: number;
          name: string;
          departmentId: number;
