...
export default async function (fastify: FastifyInstance) {
	fastify.get("/", async (req: FastifyRequest, reply: FastifyReply) => {
		try {
            ...
            const company = await prisma.companies.findUnique({ where: { id: Number(id) } });
	...
	fastify.post("/upload-image", async (req: FastifyRequest, reply: FastifyReply) => {
		try {
			...
			const filePath = path.join(cdnPath, 'images', 'companies', `${hash}.${ext}`);
	...
	fastify.post("/update", async (req: FastifyRequest, reply: FastifyReply) => {
		try {
			const { id, name, cnpj, address, color, image } = req.body as { id: number, name?: string, cnpj?: string, address?: string, color?: string, image?: string };
