...
export default async function (fastify: FastifyInstance) {
    fastify.get("/", async (req: FastifyRequest, reply: FastifyReply) => {
        try {
            ...
            if (user.admin) {
                userData = await prisma.admins.findUnique({
                    where: { id: user.id },
                });
            } else {
        ...
        async (req: FastifyRequest, reply: FastifyReply) => {
            try {
                ...
                if (existingCpf)
                    return reply
                        .status(400)
                        .send({ error: "cpf-already-registered" });
                ...
                reply.setCookie("mastersight-access", token, {
                    httpOnly: true,
                    path: "/",
                });
    ...
    fastify.post(
        ...
        async (req: FastifyRequest, reply: FastifyReply) => {
            try {
                ...
                const { name, cpf, birthday, image, description } =
                    req.body as {
                        [key: string]: string;
                    };
    ...
    fastify.get(
        ...
        async (req: FastifyRequest, reply: FastifyReply) => {
            try {
                ...
                const invites = await prisma.companiesUsers.findMany({
