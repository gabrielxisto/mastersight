...
export default function () {
  ...
  const [defaultValues, setDefaultValues] = useState<UserFormValues>({
    ...
    birthday: user.current?.birthday
      ? new Date(user.current.birthday)
      : new Date(),
    ...
    if (!result.success) {
      ...
      api.post("/users/update", formValues).then((response) => {
        if (response.status === 200) {
          ...
          setIsDisabled(true);
          setErrors({});
          toast.success(messages.success["user-updated"]);
        }
      });
    }
  };
    ...
    <main className={cn("flex flex-col justify-center px-17 py-14")}>
      <div className="space-y-0.5 w-full">
        ...
        <p className="text-muted-foreground">
          Gerencie e altere as configurações da sua conta para as que você
          melhor preferir.
        </p>
      </div>
      ...
      <div className="flex flex-1 flex-col gap-8 space-y-2 overflow-hidden md:space-y-2 lg:space-y-0 lg:space-x-12">
        <div className="flex gap-9 w-full overflow-y-hidden p-1">
          <Infos
            ...
            setErrors={setErrors}
          />
          <Appearance data={formValues} set={setFormValues} />
        </div>
