    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[26.56rem]">
        <form className="flex flex-col gap-6" onSubmit={onSubmit}>
          <DialogHeader>
            <DialogTitle className="text-red-500">Deletar competência</DialogTitle>
            <DialogDescription>
              Tem certeza que deseja deletar a competência <b className="uppercase">{data?.name}</b>? Esta ação não pode ser desfeita.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4">
            <div className="grid gap-3">
              <Label htmlFor="competence-title">Titulo</Label>
              <Input 
                id="competence-title" 
                name="title" 
                required 
                placeholder="Insira o titulo da competência para confirmar"
