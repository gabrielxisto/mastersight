...
function AuthComponent() {
  ...
  useEffect(() => {
    if (token) {
      api.get(`/auth/validate-token?token=${token}`).then(() => {
        router.navigate({ to: '/auth/reset', search: (old) => ({ ...old, token }) })
      }).catch(() => {
        router.navigate({ to: '/auth', search: (old) => ({ ...old, token: undefined }) })
      })
    }
  }, [token])
    ...
    <div className="grid min-h-svh lg:grid-cols-2 bg-background">
      <div className="flex flex-col gap-4 p-6 md:p-10">
        <div className="flex justify-center gap-2 md:justify-start">
          <a href="https://mastersight.com" className="flex items-center gap-2 font-medium text-foreground">
            <svg 
