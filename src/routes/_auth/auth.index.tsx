...
function AuthComponent({
  ...
  const signIn = (e: React.FormEvent<HTMLFormElement> | null, type: "credentials" | "google") => {
    if (type === "google") {
      ...
      if (e) {
        ...
        api.post("/auth/credentials", { email, password, type: "users" }).then((response) => {
  ...
  return (
    <form onSubmit={(e) => signIn(e, "credentials")} className={cn("flex flex-col gap-6", className)} {...props}>
